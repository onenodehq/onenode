...
from blueprints.v0.utils.resource_helper import (
...
from blueprints.v0.utils.mongo_setup import mongo_onenode_contents
...
def get_resource_service(request, user_id):
    ...
    if is_admin:
        data = mongo_onenode_contents.find({}, projection={"_id": 0}).sort(
            "created_at", pymongo.DESCENDING
        )
    elif id:
        data = mongo_onenode_contents.find(
            {"user_id": user_id, "_id": id}, projection={"_id": 0}
        ).sort("created_at", pymongo.DESCENDING)
    else:
        data = mongo_onenode_contents.find({"user_id": user_id}, projection={"_id": 0}).sort(
            "created_at", pymongo.DESCENDING
        )
    ...
    for item in data:
        if item.get("type").startswith("image/"):
            ...
            if not signed_url or is_signed_url_expired(signed_url=signed_url):
                ...
                mongo_onenode_contents.update_one(filter=filter, update=update)
...
def create_resource_service(request, user_id):
    ...
    resources = process_image_resources(resources=resources, ids=ids, user_id=user_id)
    ...
    mongo_onenode_contents.insert_many(documents=mongo_documents)
...
def update_resource_service(request):
    ...
    for resource in resources:
        ...
        mongo_onenode_contents.update_one(
            {"id": id},
            {
                "$set": {
                    "text": content,
...
def delete_resource_service(request, user_id):
    ...
    data: List = list(mongo_onenode_contents.find(query))
    s3_keys = []
    for item in data:
    ...
    filter = {"id": {"$in": ids}}
    mongo_onenode_contents.delete_many(filter)
...
def update_resource_context_service(user_id: str, request, context_id: str):
    ...
    mongo_onenode_contents.update_one({"_id": context_id, "user_id": user_id}, update_data)

    current_docs = mongo_onenode_contents.find({"user_id": user_id})
    updates = []
    ...
    if updates:
        mongo_onenode_contents.bulk_write(updates)
