...
def process_document_fields(
    ...
    for key, value in dict.items():
        ...
        if key == "@text":
            if not EmbText.is_valid_data(data=dict):
                abort(400, description=f"Field value is invalid - {dict}")

            text = value.get("text")
            chunks = chunk(text=text)
            dict[key]["chunks"] = chunks
            pc_ids = create_pc_ids(
                path=full_path, length=len(chunks), document_id=document_id
            )
...
def create_vectors(
    embeddings: list, pc_ids: list[str]
) -> list:
...
def update_pc(
    ...
    for document_id in document_ids:
        vectors = create_vectors(
            embeddings=embeddings, metadatas=pc_ids, document_id=document_id
        )
    ...
    pc_client_delete_with_prefixes(prefixes=delete_id_prefixes)
