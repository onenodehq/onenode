...
def get_chunk_by_path(doc: dict, path: str, chunk_n: int) -> str | None:
    # Paths are generated by process_document() with dot notation, including
    # numeric list indexes such as "items.0.description".
    path_segments = path.split(".") if path else []

    # Step 2: Traverse the document based on path segments
    current_value = doc
    for segment in path_segments:
        if isinstance(current_value, dict):
            current_value = current_value.get(segment, {})
        elif isinstance(current_value, list) and segment.isdigit():
            index = int(segment)
            current_value = (
                current_value[index] if 0 <= index < len(current_value) else {}
            )
        else:
            current_value = {}
            break
    ...
    emb_dict = (
        current_value.get("xImage") or current_value.get("xText")
        if isinstance(current_value, dict)
        else None
    )
